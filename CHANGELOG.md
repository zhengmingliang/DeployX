...
- **部署后系统通知**：批量上传/部署完成后，当 IDE 窗口未聚焦时弹出 OS 级系统通知（Linux 走 libnotify，macOS 走通知中心，Windows 走系统托盘），同时始终显示 IDE 气泡通知（可在设置中开关）。改用 IntelliJ Platform 的 `SystemNotifications` API，弃用不可靠的 `java.awt.SystemTray`（旧实现因 SVG 图标无法被 ImageIcon 加载而静默失败）
