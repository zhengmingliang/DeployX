...
### ✨ 新功能
- **侧边栏工具窗口**：远程文件浏览器改为非模态侧边栏工具窗口（`DeployX Remote`，停靠在左侧），浏览远程文件不再阻塞本地项目编辑
- **IDE 编辑器集成**：双击远程文件直接在 IDE 主编辑器中打开，完整语法高亮；`Ctrl+S` 自动将修改写回远程服务器（修复此前弹窗内容为空的 bug）。超过 5MB 的文件拒绝打开并提示改用下载
- **自动映射与路径记忆**：首次打开时按当前项目路径匹配映射，自动定位到对应远程目录；每台服务器上次打开的目录会被记忆，下次打开自动恢复
- **路径历史**：路径输入框按服务器记忆成功进入过的路径（最多 20 条）；历史按钮弹出菜单可快速跳转、删除单条或清空全部历史
- **拖拽上传**：从项目视图或系统文件管理器拖拽本地文件/目录到浏览器树，自动上传到拖放目标远程目录（成功后自动刷新）
- **拖拽下载**：从浏览器树拖拽远程文件/目录到项目视图（或任意文件放置目标），直接下载到项目中

### 🐛 Bug 修复
- **远程文件编辑弹窗内容为空**：废弃原有弹窗编辑器，改为在 IDE 主编辑器中打开远程文件（基于 `LightVirtualFile` + 文件类型识别语法高亮），并通过 `FileDocumentManagerListener` 在保存时自动写回远程，彻底解决内容为空的问题
...
### ✨ 新功能
- **远程文件浏览器**：在 IDE 内以树形结构直接浏览远程服务器文件系统，支持以下能力：
  - **查看**：懒加载目录树，展开时按需拉取子目录；显示文件大小、修改时间与图标；支持切换服务器、路径跳转、上级目录、刷新
  - **编辑**：双击远程文本文件即可在弹出的编辑器中查看/修改内容，点击「保存」后台写回远程文件；超过 10MB 的大文件会提示改用下载，避免阻塞 UI；关闭时有未保存修改会二次确认
  - **下载**：右键可将远程文件或整个目录递归下载到本地指定位置，下载完成后显示文件数与总大小
  - **目录管理**：支持在浏览器内新建目录、删除文件/目录（非空目录通过 `rm -rf` 递归删除）、复制远程路径到剪贴板、刷新指定目录
  - 持久化 SFTP 通道复用，展开目录无需重复建立 SSH 连接，浏览延迟显著降低；通道断开时自动重连
