        ...
        changeNotes.set(
            """
                ...
                <ul>
                    ...
                    <li><b>🔔 System Notifications:</b> After batch upload/deploy completes, an OS-level notification is shown when the IDE window is not focused (Linux via libnotify, macOS via Notification Center, Windows via system tray), plus an IDE balloon is always shown. Configurable in settings. Now uses the IntelliJ Platform <code>SystemNotifications</code> API instead of the unreliable <code>java.awt.SystemTray</code> (the old impl silently failed because the SVG icon could not be loaded by ImageIcon)</li>
                ...
                <ul>
                    ...
                    <li><b>🔔 部署后系统通知：</b>批量上传/部署完成后，当 IDE 窗口未聚焦时弹出 OS 级系统通知（Linux 走 libnotify，macOS 走通知中心，Windows 走系统托盘），同时始终显示 IDE 气泡通知（可在设置中开关）。改用 IntelliJ Platform 的 <code>SystemNotifications</code> API，弃用不可靠的 <code>java.awt.SystemTray</code>（旧实现因 SVG 图标无法被 ImageIcon 加载而静默失败）</li>
